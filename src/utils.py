...
def gaussian_kernel(size, sigma):
    ...
    kernel /= kernel.sum() # Weighted averaging
    return kernel
...
class SSIM_Loss(_Loss):
    def __init__(self, in_channels, size = 5, sigma = 1.5, size_average = True):
        ...
        weight = np.tile(kernel, (in_channels, 1, 1, 1))   # (1,1,11,11)
        self.weight = Parameter(torch.from_numpy(weight).float(), 
                                requires_grad = False)   # todo

    def forward(self, img1, img2):
       # print(img1.shape,img2.shape) # (batch_size, 1, H, W)
        mean1 = F.conv2d(img1, self.weight, padding = self.size, 
                         groups = self.in_channels)
       # print(mean1.shape)
        mean2 = F.conv2d(img2, self.weight, padding = self.size, 
                         groups = self.in_channels)
       # print(mean2.shape)
        ...
        sigma1_sq = F.conv2d(img1 * img1, self.weight, 
                             padding = self.size, 
                             groups = self.in_channels) - mean1_sq # padding = 0
        sigma2_sq = F.conv2d(img2 * img2, self.weight, 
                             padding = self.size, 
                             groups = self.in_channels) - mean2_sq
        sigma_12 = F.conv2d(img1 * img2, self.weight, 
                            padding = self.size, 
...
def visualization_save(path, img, gt_density_map, et_density_map, 
    ...
    label_visualization(label5, et_density_imap, 50, p3)
